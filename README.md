...
[![版本](https://img.shields.io/badge/Version-0.6.4-success?style=flat-square)](https://github.com/ROOT94-MAX/DiscordAIMessageCleaner)
...
**当前版本：v0.6.4** · **运行环境：BetterDiscord（无需第三方库）**
...
- **安全删除：**二次确认 → 可选备份 → 节流删除，进度可暂停 / 取消，结束给出成功 / 跳过 / 失败报告并可导出删除记录。
...
4. **删除**：点「删除选中」→ 二次确认（明示不可逆）→ 按设置决定是否先备份 → 节流删除，结束给出报告。
...
- 删除**不可逆**。默认「先审后删 + 人工勾选 + 二次确认」，绝不静默删除；建议首次把备份模式设为「每次询问」，先导出 JSON 再删。
