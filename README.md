...
- 设置板块与总结插件保持一致：移除分割线，分组标题使用 `24px 0 8px` 外边距（首组顶部为 0）；字段行与表单项继续保持紧凑。
- 插件界面语言从“审查策略”移到“通用设置”；“诊断”升级为“关于与诊断”，About 卡片展示插件简介、版本标签和可点击的 GitHub 图标，下方保留运行诊断与复制诊断信息。
- “内容”字段标题单独降为 15px/20px 行高，并使用 6px 标题到文本框间距，不再与分组标题争夺层级；“运行诊断”的说明收进标题后的信息图标。
- About 卡片提供手动“检查更新”：仅查询官方 GitHub 最新稳定版；发现新版本后可查看说明或确认“下载并安装”。安装前校验官方资产 URL、GitHub SHA-256、插件名和版本，备份当前插件，写入后再次校验；失败自动恢复，不后台静默更新、不降级候选版本。
