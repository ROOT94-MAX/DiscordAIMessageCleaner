...
- 清理结果列表对齐后台弹窗视觉层级：频道徽章弱化，选中行轻微着色；动态自定义表情带 GIF/WebP/PNG 回退，图片附件直接预览，其他附件保留文件信息，正文链接与附件链接可点击。
- 消息跳转改用客户端原生频道选择与 `jumpToMessage`；同频道和跨频道均在客户端内定位，不再通过网页或整页重载跳转。
- 服务器范围扫描按 `guild:<guildId>` 缓存在当前插件运行期，同一服务器切换频道后可恢复结果、筛选与手动勾选；频道和私信按 `channel:<channelId>` 隔离，最多保留最近 20 个作用域。
- 删除确认改为明确的“取消 / 永久删除”按钮；删除或已不存在的消息会同步从扫描缓存、后台审查会话和当前选择中移除。
